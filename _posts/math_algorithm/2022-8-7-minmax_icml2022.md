...
来表示分布$D$下的期望损失。那么所谓的min-max，即寻找一个最优的模型$f_{\theta^*}$，
使得所有组别的损失的最大值$\max_{i\in [g]}v(\theta^*; D_i)$最小，即
...
> 
> 3:     compute $i_t = \argmax_{i\in [g]} v(\theta_t; \hat{D}_i)$
> 
...
$$
Proj_\Theta(\theta_1) = \argmin_{\theta \in \Theta} \lVert\theta - \theta_1 \rVert_2
$$
...
\text{(apply OGD regret bound)} & \le \mathbb{E}_{z_{1:T}}[\frac{1}{T}  \sum_{t=1}^T l(f_{\theta^*};z_t) + \frac{\eta L^2}{2} + \frac{W^2}{2T\eta}] + 2R_\delta \\
\text{($\eta=\frac{W}{L\sqrt{T}}$)} & = \mathbb{E}_{z_{1:T}}[v(\theta^*;\{z_1, z_2, \ldots, z_T\})] + \frac{WL}{\sqrt{T}} + 2R_\delta \\
\text{(average $\le$ maximum)} & \le max_{i \in [g]} v(\theta^*; D_i) + \frac{WL}{\sqrt{T}} + 2R_\delta
\end{align*}
$$
...
> $$
> \text{Regret}_T = \sum_{t=1}^T f_t(x_t) - \min_{x^* \in K}\sum_{t=1}^T f_t(x^*) \le \frac{3}{2} WL\sqrt{T}.
> $$
...
$$
\lVert x_{t+1} - x^* \rVert^2 = \lVert Proj_K(x_t -\eta_t \nabla f_t(x_t)) - x^* \rVert^2 \le \lVert x_t - \eta_t \nabla f_t(x_t) - x^* \rVert^2
$$

展开右边的平方项，得到

$$
\lVert x_{t+1} - x^* \rVert^2 \le \lVert x_t - x^* \rVert^2 + \eta_T^2 \lVert \nabla f_t(x_t) \rVert^2 - 2\eta_t \nabla^T f_t(x_t) (x_t - x^*)
$$

用上界$L$替换掉$\lVert \nabla f_t(x_t) \rVert^2$，即有

$$
2\nabla^T f_t(x_t) (x_t - x^*) \le \frac{\lVert x_t - x^* \rVert^2 - \lVert x_{t+1} - x^* \rVert^2}{\eta_t} + \eta_t L^2
$$

式子左边的项实际上是非常眼熟的，它经常出现在凸集的定义当中。由于我们的空间$K$是凸的，因此有

$$
\nabla^T f_t(x_t) (x_t - x^*) \ge f_t(x_t) - f_t(x^*)
$$

将上述式子利用$\nabla^T f_t(x_t) (x_t - x^*)$串起来，得到

$$
2\sum_{t=1}^T (f_t(x_t) - f_t(x^*)) \le \sum_{t=1}^T \frac{\lVert x_t - x^* \rVert^2 - \lVert x_{t+1} - x^* \rVert^2}{\eta_t} + L^2 \sum_{t=1}^T \eta_t
$$

考虑到$\lVert x_{T+1} - x^* \rVert^2 \ge 0$，我们将$\frac{\lVert x_{T+1} - x^* \rVert^2}{\eta_T}$从和式中拿出，得到

$$
2\sum_{t=1}^T (f_t(x_t) - f_t(x^*)) \le \sum_{t=1}^T \lVert x_t - x^* \rVert^2 (\frac{1}{\eta_t} - \frac{1}{\eta_{t-1}}) + L^2 \sum_{t=1}^T \eta_t
$$

并用直径$W$替换到$\lVert x_t - x^* \rVert^2$

$$
2\sum_{t=1}^T (f_t(x_t) - f_t(x^*)) \le \sum_{t=1}^T D^2 (\frac{1}{\eta_t} - \frac{1}{\eta_{t-1}}) + L^2 \sum_{t=1}^T \eta_t \le \sum_{t=1}^T D^2 \frac{1}{\eta_T} + L^2 \sum_{t=1}^T \eta_t
$$
...
$$
2\sum_{t=1}^T (f_t(x_t) - f_t(x^*)) \le 3DG\sqrt{T}
$$
...
$\text{KL}(p \mid \mid q) = \sum_{i=1}^g$ p(i) \log \frac{p(i)}{q{i}}表示KL散度，以及使用$q^*$作为$\argmax_{i \in [g]}v(\overline \theta_T;\hat D_i)$的指示分布。则有 -->
