...
(3) 若有子集$F_1,F_2 \in \mathcal{F}, |F_1| < |F_2|$，则存在一个元素$a \in F_2 \setminus F_1$使得$F_1 \cup \{a\} \in \mathcal{F}$。（通常我们称之为交换性（Exchange Property））
...
假定有子集$F_1, F_2 \in \mathcal{F}, |F_1| < |F_2|$，则由线性无关的定义可知，以$F_1$作为基（Base）的向量空间（Vector Space）$V_1$的维度为$|F_1|$，而以$F_2$作为基的向量空间$V_2$的维度为$|F_2|$。由于$|F_2| > |F_1|$，因而一定至少有一个$V_2$中的向量$\vec{v}$不存在与$V_1$中，即无法表示为$F_1$中向量的线性组合，因而$\{\vec{v}\} \cup F_1$也是一组线性无关的向量，交换性得证。
...
**证明：** 假定有子集$F_1, F_2 \in \mathcal{F}, |F_1| < |F_2|$，由于$G[F_1]$与$G[F_2]$均为树，这意味着它们的顶点数分别为$|F_1|+1$与$|F_2|+1$。很显然我们有$|F_2|+1 > |F_1|+1$，这意味着一定存在一个顶点$u \in V(F_2) \setminus V(F_1)$，那么我们便可把$F_2$中连接点$u$的边（很显然这条边是不存在于$F_1$之中的）加入到$|F_1|$之中，从而得到一个新的树。交换性得证。
...
(2) 集合$F$由空集$\phi$开始，依次向$F$中按照权重由小到大的顺序添加使得$F \cup \{e_i\} \in \mathcal{F}$的$e_i, i \in [1, |E|]$。
...
**证明：** 这里使用反证法，假设存在两个基$F_1, F_2$大小不一。不妨设$|F_2| > |F_1|$。
...
对于一个独立集合$F$来说，若定义$f(|F|)$为检验$F \cup \{e_i\}$是否为独立集的时间，那么贪心算法的时间复杂度便为$\mathcal{O}(|E|log|E| + |E|f(|E|))$。
...
**证明：** 我们使用反证法来证明我们想要的结论。假设$S$中存在两个独立集合$F_1$与$F_2$使得$|F_2| > |F_1|$且不满足公理三，即对于每一个$a \in F_2 \setminus F_1$，$F_1 \cup \{a\}$都不是独立集。
...
w(a)=\left\{
\begin{array}{ll}
w_1 = \epsilon / |E| & \text{if $a \in F_1 \cap F_2$} \\
w_2 = \epsilon / |F_1 \setminus F_2| & \text{if $a \in F_1 \setminus F_2$}\\
w_3 = (1 + \epsilon) / |F_2 \setminus F_1| & \text{if $a \in F_2 \setminus F_1$}\\
w_4 = 2 & \text{otherwise}
\end{array}
\right.
$$
...
其中$0 < \epsilon < 1$。
很自然的我们有$w_1 < w_2$且$w_3 < w_4$。而由$|F_2| > |F_1|$可知$w_3 w_2$。即有$w_1 < w_2 < w_3 < w_4$。
...
$$
\begin{align}
|F_1 \cap F_2|w_1 + |F_1 \setminus F_2|w_2 + (m - |F_1|)w_4 > \\
|F_1 \cap F_2|w_1 + |F_2 \setminus F_1|w_3 + (m - |F_2|)w_4
\end{align}
$$
...
$$
(|F_2| - |F_1|)w_4 > |F_2 \setminus F_1|w_3 - |F_1 \setminus F_2|w_2
$$
...
\begin{align}
w_4 &> 1 \\
&= \frac{|F_2 \setminus F_1|(1 + \epsilon)}{|F_2 \setminus F_1|} - \frac{|F_1 \setminus F_2|\epsilon}{|F_1 \setminus F_2|} \\
&= |F_2 \setminus F_1|w_3 - |F_1 \setminus F_2|w_2
\end{align}
$$

回忆起$|F_2| > |F_1|$，即有$|F_2| - |F_1| \ge 1$，很自然的便能得到我们的结论。
