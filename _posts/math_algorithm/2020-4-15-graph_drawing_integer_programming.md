...
<figure style="text-align:center;">
  <img src="{{ site.BASE_PATH }}/assets/images/gd_1_6.png" width="500" height="350" alt="1"/>
</figure>
...
至此，我们的生成树便决定好了。注意到在此语境下，实际上我们的生成树是一棵有根树，因为只选择$|V'| - 1$条边的情况下，一定会有一个点的入度为$0$，而其也就是事实上的根。考虑到对于决定DFS序而言，我们实际上是需要一个起点的，一棵有根树可谓是正中下怀。
...
$$
z_{uv} + z_{vw} - z_{uw} <= 1, \forall u, v, w \in V'
$$
...
$$
x_{u_ev_e, p_fq_f} >= z_{u_ev_e} + z_{p_fq_f} + z_{u_ep_f} + z_{p_fv_e} + z_{v_eq_f} - 4, \forall e = uv, f = pq \in E
$$
